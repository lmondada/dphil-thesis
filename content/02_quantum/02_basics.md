...
operations that _modify_---or "mutate", to borrow a term from
...
are an excellent foundation on which to develop quantum-specific computation
optimisations and compilation techniques in general.
...
In {{% reflink "chap:matching" %}} we will see that it greatly simplifies pattern matching,
...
must consider, resulting in elegant graph transformation semantics---a topic
we explore in {{% reflink "chap:compiler" %}}.
...
us astray.
The references in {{% reflink "sec:litt" %}} are a good starting point
for further reading.
